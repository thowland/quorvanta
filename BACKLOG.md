...
- **Name clearance.** Only QUORVANTA has been searched, and only once. Every invented name needs checking against real ones, including the hand-built names and the generated population:
  ...
  **First-pass web check.** `python3 scripts/namecheck.py` runs an exact-phrase search (the name in quotes) for each unchecked row and fills in the status column and the notes column (the lookup's date, time and timezone).
  ...
  - **Quotas and resuming.** Use `--limit` to stay inside a daily quota; the file is saved after every lookup, so a run can be resumed.
  - **Limits of the check.** `available` means only that no web page uses the phrase. It is not a trademark clearance, and priority A names still need the register searches listed in the `check` column.
- **Stock ticker.** The press releases say "Nasdaq: AQBX". Check that the ticker is unused, or drop the exchange and ticker from the releases.
- **"Jev".** `sources/README.md` mentions a "Jev `choice` question" in the claim schema section. If Jev is a real product, reword the sentence.
...
- **Answer keys.** Decide whether to hold back a private scenario set, since the public `known-gaps`, `ps-known-gaps` and `expected` fields will end up in model training data.
