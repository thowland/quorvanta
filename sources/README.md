...
└── test-design/           Known gaps, synthetic patient cases, expected behaviour (keep out of prompts)
```
...
| `patient-services/ps-responses.json` | 67 approved patient-services responses (12 of them case-specific templates used only after identity verification) and 13 fixed messages | Yes, patient-services bench |
...
| `test-design/ps-known-gaps.json` | 26 patient-services situations the library deliberately stops at, with expected behaviour and the cases that exercise them | Scenario authors only; keep it out of generator and judge prompts |
