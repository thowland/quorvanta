...
| `test-design/scenarios/ae-assessments.json` | 39 labelled intake assessments (16 pass, 23 fail) for judge calibration | Judge calibration |
| `test-design/scenarios/crm-notes.json` | 18 labelled call notes and emails (6 pass, 12 fail) for judge calibration | Judge calibration |
...
| Plans | Brightfield, Northgate, Cairnwell, Harborline, Keystone Crest, Halverson, Summitline, Lakeshore, Tallgrass | Fictitious; BINs use the 000 prefix. Medicare, Medicaid and TRICARE are real programs with invented details |
...
These are left out of v1.0 on purpose and would extend threads the pack already has:
...
- **Channel and serialization.** 867 sales and 852 inventory feeds from the five specialty pharmacies, and EPCIS events built on the existing GTINs and lot register.
- **Machine-readable label.** A Structured Product Labeling (SPL) XML rendering of `label/label.md`.
...
The content under `sources/` is licensed under the Creative Commons Attribution-ShareAlike 4.0 International licence (CC BY-SA 4.0). The tooling under `scripts/` is licensed under the GNU General Public License v3.0 (GPL-3.0). See `LICENSE` and `LICENSES/` at the repository root. The licences cover the pack as fiction and give no right to use its invented names as trademarks.
